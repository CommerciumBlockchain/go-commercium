...
import (
	...
	"github.com/CommerciumBlockchain/go-commercium/accounts/abi/bind"
	"github.com/CommerciumBlockchain/go-commercium/accounts/abi/bind/backends"
	"github.com/CommerciumBlockchain/go-commercium/common"
	"github.com/CommerciumBlockchain/go-commercium/common/mclock"
	"github.com/CommerciumBlockchain/go-commercium/consensus/ethash"
	"github.com/CommerciumBlockchain/go-commercium/contracts/checkpointoracle/contract"
	...
	"github.com/CommerciumBlockchain/go-commercium/event"
	"github.com/CommerciumBlockchain/go-commercium/les/checkpointoracle"
	"github.com/CommerciumBlockchain/go-commercium/les/flowcontrol"
	"github.com/CommerciumBlockchain/go-commercium/light"
	"github.com/CommerciumBlockchain/go-commercium/p2p"
	"github.com/CommerciumBlockchain/go-commercium/p2p/enode"
	"github.com/CommerciumBlockchain/go-commercium/p2p/nodestate"
...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			tx, _ := types.SignTx(types.NewTransaction(nonce, userAddr1, big.NewInt(10000), nil, nil), signer, bankKey)
		...
		case 1:
			...
			tx1, _ := types.SignTx(types.NewTransaction(bankNonce, userAddr1, big.NewInt(1000), nil, nil), signer, bankKey)
			backend.SendTransaction(ctx, tx1)

			// user1 relays ether to user2
			tx2, _ := types.SignTx(types.NewTransaction(userNonce1, userAddr2, big.NewInt(1000), nil, nil), signer, userKey1)
		...
		case 2:
			...
			bankNonce, _ := backend.PendingNonceAt(ctx, bankAddr)
			tx1, _ := types.SignTx(types.NewTransaction(bankNonce, signerAddr, big.NewInt(1000000000), nil, nil), signer, bankKey)
			backend.SendTransaction(ctx, tx1)
