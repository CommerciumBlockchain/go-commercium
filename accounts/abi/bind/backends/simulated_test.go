...
func TestSimulatedBackend(t *testing.T) {
	...
	tx := types.NewContractCreation(0, big.NewInt(0), big.NewInt(1), common.FromHex(code))
...
func TestNewSimulatedBackend_AdjustTimeFail(t *testing.T) {
	...
	sim := simTestBackend(testAddr)
	// Create tx and send
	tx := types.NewTransaction(0, testAddr, big.NewInt(1000), big.NewInt(1), nil)
	...
	tx2 := types.NewTransaction(1, testAddr, big.NewInt(1000), big.NewInt(1), nil)
...
func TestSimulatedBackend_NonceAt(t *testing.T) {
	...
	tx := types.NewTransaction(nonce, testAddr, big.NewInt(1000), big.NewInt(1), nil)
	signedTx, err := types.SignTx(tx, types.HomesteadSigner{}, testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
...
func TestSimulatedBackend_SendTransaction(t *testing.T) {
	...
	// create a signed transaction to send
	tx := types.NewTransaction(uint64(0), testAddr, big.NewInt(1000), big.NewInt(1), nil)
	signedTx, err := types.SignTx(tx, types.HomesteadSigner{}, testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
...
func TestSimulatedBackend_PendingNonceAt(t *testing.T) {
	...
	tx = types.NewTransaction(uint64(1), testAddr, big.NewInt(1000), big.NewInt(1), nil)
	signedTx, err = types.SignTx(tx, types.HomesteadSigner{}, testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
