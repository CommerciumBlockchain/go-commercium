...
func NewContractCreation(nonce int64, amount *BigInt, data []byte) *Transaction {
	return &Transaction{types.NewContractCreation(uint64(nonce), amount.bigint, common.CopyBytes(data))}
}
...
func NewTransaction(nonce int64, to *Address, amount *BigInt, data []byte) *Transaction {
	if to == nil {
		return &Transaction{types.NewContractCreation(uint64(nonce), amount.bigint, common.CopyBytes(data))}
	}
	return &Transaction{types.NewTransaction(uint64(nonce), to.address, amount.bigint, common.CopyBytes(data))}
}
