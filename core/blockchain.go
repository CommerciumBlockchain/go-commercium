...
import (
	...
	"github.com/CommerciumBlockchain/go-commercium/common/prque"
	...
	"github.com/CommerciumBlockchain/go-commercium/core/state/snapshot"
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals bool) (int, error) {
	...
	for ; block != nil && err == nil || err == ErrKnownBlock; block, err = it.next() {
		...
		substart := time.Now()
		receipts, logs, err := bc.processor.Process(block, statedb, bc.vmConfig)
		if err != nil {
		...
		substart = time.Now()
		if err := bc.validator.ValidateState(block, statedb, receipts); err != nil {
			bc.reportBlock(block, receipts, err)
			atomic.StoreUint32(&followupInterrupt, 1)
			return it.index, err
		}
		...
		case CanonStatTy:
			log.Debug("Inserted new block", "number", block.Number(), "hash", block.Hash(),
				"uncles", len(block.Uncles()), "txs", len(block.Transactions()),
		...
		case SideStatTy:
			log.Debug("Inserted forked block", "number", block.Number(), "hash", block.Hash(),
				...
				"txs", len(block.Transactions()), "uncles", len(block.Uncles()),
				"root", block.Root())
...
func (bc *BlockChain) reportBlock(block *types.Block, receipts types.Receipts, err error) {
	...
	for i, receipt := range receipts {
		receiptString += fmt.Sprintf("\t %d: contract: %v status: %v tx: %v logs: %v bloom: %x state: %x\n",
			i, receipt.ContractAddress.Hex(),
