...
func transaction(nonce uint64, key *ecdsa.PrivateKey) *types.Transaction {
	return pricedTransaction(nonce, big.NewInt(1), key)
}

func pricedTransaction(nonce uint64, key *ecdsa.PrivateKey) *types.Transaction {
	tx, _ := types.SignTx(types.NewTransaction(nonce, common.Address{}, big.NewInt(100), nil), types.HomesteadSigner{}, key)
	return tx
}

func pricedDataTransaction(nonce uint64, key *ecdsa.PrivateKey, bytes uint64) *types.Transaction {
	data := make([]byte, bytes)
	rand.Read(data)

	tx, _ := types.SignTx(types.NewTransaction(nonce, common.Address{}, big.NewInt(0), data), types.HomesteadSigner{}, key)
	return tx
}
...
func TestInvalidTransactions(t *testing.T) {
	...
	balance := new(big.Int).Add(tx.Value(), new(big.Int).Mul(new(big.Int)))
	pool.currentState.AddBalance(from, balance)
	if err := pool.AddRemote(tx); !errors.Is(err) {
