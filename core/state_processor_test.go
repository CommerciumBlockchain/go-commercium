...
func TestStateProcessorErrors(t *testing.T) {
	...
	var makeTx = func(nonce uint64, to common.Address, amount *big.Int, data []byte) *types.Transaction {
		tx, _ := types.SignTx(types.NewTransaction(nonce, to, amount, data), signer, testKey)
		return tx
	}
	for i, tt := range []struct {
		...
		{
			txs: []*types.Transaction{
				makeTx(0, common.Address{}, big.NewInt(0), nil, nil),
				makeTx(0, common.Address{}, big.NewInt(0), nil, nil),
			},
		...
		{
			txs: []*types.Transaction{
				makeTx(100, common.Address{}, big.NewInt(0), nil, nil),
			},
		...
		{
			txs: []*types.Transaction{
				makeTx(0, common.Address{}, big.NewInt(1), nil, nil),
			},
		...
		{
			txs: []*types.Transaction{
				makeTx(0, common.Address{}, big.NewInt(0), big.NewInt(0xffffff), nil),
			},
		...
		},
		{
			txs: []*types.Transaction{
				makeTx(0, common.Address{}, big.NewInt(0), nil, nil),
				makeTx(1, common.Address{}, big.NewInt(0), nil, nil),
				makeTx(2, common.Address{}, big.NewInt(0), nil, nil),
				makeTx(3, common.Address{}, big.NewInt(0), big.NewInt(0), nil),
			},
...
func GenerateBadBlock(parent *types.Block, engine consensus.Engine, txs types.Transactions) *types.Block {
	...
	for _, tx := range txs {
		txh := tx.Hash()
		hasher.Write(txh[:])
		receipt := types.NewReceipt(nil, false)
